...
function usedPropTypesAreEquivalent(propA, propB) {
  return propA.name === propB.name && propA.allNames.join(':') === propB.allNames.join(':');
}
...
module.exports = function usedPropTypesInstructions(context, components, utils) {
  ...
  function markPropTypesAsUsed(node, parentNames) {
    ...
    const component = components.getParentComponent(node);
    if (!component) {
      return;
    }
    const usedPropTypes = component.usedPropTypes || [];
    let ignoreUnusedPropTypesValidation = component.ignoreUnusedPropTypesValidation || false;
    ...
    components.set(component.node, {
      usedPropTypes: mergeUsedPropTypes(component.usedPropTypes || [], usedPropTypes),
