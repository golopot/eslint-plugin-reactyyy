...
module.exports = function usedPropTypesInstructions(context, components, utils) {
  ...
  /**
   * Find the destructuring part from nodes like `function SomeComponent({ foo }) {...}`.
   * @param {ASTNode} node A function node
   * @returns {ASTNode | undefined} An ObjectPattern if found
   */
  function findPropsDestructuring(node) {
    const param = isSetStateUpdater(node) ? node.params[1] : node.params[0];
  ...
  function handleFunctionLikeExpressions(node) {
    pushScope();
    const objectPattern = findPropsDestructuring(node);
    ...
    if (
      ...
      components.get(node) /* function component */ ||
      components.get(node.parent) /* React.memo(...) */ ||
      isNodeALifeCycleMethod(node.parent, checkAsyncSafeLifeCycles)
    ) {
      markPropTypesAsUsed(objectPattern);
    }
  }
  ...
  function handleCustomValidators(component) {
    ...
    Object.keys(propTypes).forEach((key) => {
      ...
      if (node.value && astUtil.isFunctionLikeExpression(node.value)) {
        const objectPattern = findPropsDestructuring(node.value);
