 ...
 * @param {object[]} rules the returned value a eslint rule.create(context)
...
function mergeRules(rules) {
  ...
  for (const rule of rules) {
    for (const key of Object.keys(rule)) {
      ...
      handlers.get(key).push(rule[key]);
    }
  }

  function mergedHandler(key, node) {
  ...
  for (const key of handlers.keys()) {
    rule[key] = mergedHandler.bind(undefined, key);
  }
